...
class CBInfo {
  ...
  reDraw(){
    ...
    if (this.child_id != null ){
      hash_bs_info[this.child_id].reDraw(this);
    }
...
hash_cb_info["CB1"].setChildId("BS1");
hash_bs_info["BS3"] = new BSInfo("BS3");
hash_cb_info["CB2"].setChildId("BS3");

//----------------------------//
//       関数の宣言           //
//----------------------------/.
//CB情報のオンオフを変更し、CBシンボルの色も変える
function redraw(cb_img) {
  //alert();
  //クリックされたCBのIDからCBInfoを取得する
  var cb_info = hash_cb_info[cb_symbol.getAttribute("id")];
  //オンオフ状態によって色を切り替える
  if(cb_info.on_off){
    cb_symbol.setAttribute("fill","green");  
  }
  ...
  cb_symbol.ChangeOnOff();
}

function CBSymbolUpdate(cb_id) {
  //alert();
  var cb_symbol = document.getElementById(cb_id);
  //クリックされたCBのIDからCBInfoを取得する
  var cb_info = hash_cb_info[cb_id];
  //オンオフ状態によって色を切り替える
  if(cb_info.on_off == 0){
    cb_symbol.setAttribute("fill","green");  
  }
...
//TMIDを元にTMシンボルの更新処理
function TmSymbolUpdate(tm_id){
  //TMのＩＤよりTM1情報の取得
  var tm_info = hash_tm_info[tm_id];
  //指令値TMシンボルの取得
  var tm_con_symbol = document.getElementById(tm_info.getControlSymbolID());
  //指令値TMシンボルの更新
  tm_con_symbol.textContent = tm_info.getTextValControl();
  //発電量TMシンボルの取得
  var tm_now_symbol = document.getElementById(tm_info.getNowSymbolID());
  //発電量TMシンボルの更新
  tm_now_symbol.textContent = tm_info.getTextValNow();
}
